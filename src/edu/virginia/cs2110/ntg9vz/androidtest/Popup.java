...
public class Popup extends Activity {
	...
	AsyncTask<LinearLayout,LinearLayout,Double> runningGhostTask = null;
	ArrayList<LinearLayout> listOfBulletLayouts = new ArrayList<LinearLayout>();
	ArrayList<LinearLayout> listOfGhostLayouts = new ArrayList<LinearLayout>();
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		if(savedInstanceState !=  null) {
			...
			Log.d("saves", "level = " + STATE_LEVEL);
			mainCharacter.setKills(savedInstanceState.getInt(STATE_SCORE));
			mainCharacter.setRemainingHealth(savedInstanceState.getInt(STATE_HEALTH));
		} 
...
//		});
		...
		killCounter = new TextView(this);
		
		CharSequence forKillCounter = "Kills: " + mainCharacter.kills;
		killCounter.setText(forKillCounter);
		LinearLayout lKills = new LinearLayout(this);

		GridLayout.LayoutParams pKills = new GridLayout.LayoutParams(GridLayout.spec(14), GridLayout.spec(7));

		
		lKills.addView(killCounter);
		
		//Log.d("2110", "height x width of screen" + height + "\t" + width);

		gl.addView(lKills, pKills);
...
//				Log.d("2110", "height x width of layout" + l.getHeight() + "\t" + l.getWidth());
		...
		GridLayout.LayoutParams glp6 = new GridLayout.LayoutParams (GridLayout.spec(12), GridLayout.spec(7)); //Y
...
//		arrayListOfItemLayouts.add(lOrig);
	...
	public void setUpNextLevel()
	{
		...
		for(int i = 0; i < arrayOfGhostLayouts.length; i ++)
		{
			...
			listOfGhostLayouts.add(ghostLayout);
		...
		if (runningGhostTask != null) {
			Log.d("720", "Cancelling runningGhostTask");
			
			runningGhostTask.cancel(true);
		}
		
		MoveAllGhosts asyncTaskThatMovesGhosts = new MoveAllGhosts();
		runningGhostTask = asyncTaskThatMovesGhosts.execute(arrayOfGhostLayouts);
//
//		Ghost ghost2 = new Ghost(20, 8, 8, 1, 5, this);
//
//
...
//		
	...
	public void fireWeapon(Character character) { //currently only works with instances of guns
		...
		LinearLayout.LayoutParams bulletLayoutParams = new LinearLayout.LayoutParams(width/20, height/30);
		flyingLead.setLayoutParams(bulletLayoutParams);
		bulletLayout.setVerticalGravity(Gravity.CENTER);
		bulletLayout.setHorizontalGravity(Gravity.CENTER);

		bulletLayout.addView(flyingLead);

		GridLayout.LayoutParams bulletGridLayoutParams = new GridLayout.LayoutParams (GridLayout.spec(flyingLead.getyCoordinate(), GridLayout.CENTER), GridLayout.spec(flyingLead.getxCoordinate(), GridLayout.CENTER));
	...
	class MoveAllGhosts extends AsyncTask<LinearLayout, LinearLayout, Double> {
		...
		protected Double doInBackground(LinearLayout... params) 
		{
			...
			{		// while not cancelled
				...
				while(isPaused)
					sleep(5000);
				if (i % 10 == 0)				//updates every tenth of a second, but moves ghosts every 1 second
				...
				try {
					Thread.sleep(100); // sleep (wait) a tenth of a second
		...
		protected void onProgressUpdate(LinearLayout... params)
		{
			...
			int j = 0;
			//Log.i(j + "From doInBackround", "Yo this is the UI thread and i'm doing stuff  right now");
			for(LinearLayout layoutForCurrentGhost : params) 
			{
				...
				if(currentGhost.getHealth() <= 0)
				{
					if(currentGhost.isVisible())
					{
						...
						listOfGhostLayouts.remove(currentGhost);
						//if (currentGhost.isVisible)
							mainCharacter.kills ++;
						
						CharSequence newText = "Kills: " + mainCharacter.kills;
						killCounter.setText(newText);
					}				
					continue;					
				}
				...
				//alternates ghost movement
				if(xCoordinateOfThisGhost < xCoordinateOfPlayer && currentGhost.getAlternate() % 2 == 0) {
					currentGhost.changeDirection(3);
					currentGhost.setAlternate(currentGhost.getAlternate() + 1);
				}
				
				else if (xCoordinateOfThisGhost < xCoordinateOfPlayer && currentGhost.getAlternate() % 2 == 1) {
					if(yCoordinateOfThisGhost < yCoordinateOfPlayer) {
				...
				else if(xCoordinateOfThisGhost > xCoordinateOfPlayer && currentGhost.getAlternate() % 2 == 0) {
				...
				else if(yCoordinateOfThisGhost < yCoordinateOfPlayer && currentGhost.getAlternate() % 2 == 0) {
					currentGhost.changeDirection(2);
					currentGhost.setAlternate(currentGhost.getAlternate() + 1);
				}
				
				else if (yCoordinateOfThisGhost < yCoordinateOfPlayer && currentGhost.getAlternate() % 2 == 1) {
				...
				else
				{
					...
					else if (mainCharacter.getHealth() == 5) {
						healthBar.setImageResource(R.drawable.h5);
				    }
					else if (mainCharacter.getHealth() == 4) {
						healthBar.setImageResource(R.drawable.h4);
				    }
					else if (mainCharacter.getHealth() == 3) {
						healthBar.setImageResource(R.drawable.h3);
				    }
					else if (mainCharacter.getHealth() == 2) {
						healthBar.setImageResource(R.drawable.h2);
				    }
					else if (mainCharacter.getHealth() == 1) {
						healthBar.setImageResource(R.drawable.h1);
				    }
				    else 
				    {
				    	Log.d("HealthProblem", "Else statement: Health = " + mainCharacter.getHealth());
...
//		private void checkCollision(Ghost currentGhost) {
//			if(currentGhost.getxCoordinate() == mainCharacter.getxCoordinate() && currentGhost.getyCoordinate() == mainCharacter.getxCoordinate()) {
//				mainCharacter.changeHealth(currentGhost.getDamage() * -1);
//				Log.d("MainCharacter Health", mainCharacter.getHealth() + "");
//				LinearLayout healthBarLayout = (LinearLayout) findViewById(100);
//				
	...
	class MoveAllBullets extends AsyncTask<LinearLayout, LinearLayout, Double> {
		...
		protected void onProgressUpdate(LinearLayout... params){
			...
			for(LinearLayout layoutForCurrentBullet : params) 
			{
				...
				for(LinearLayout layoutForCurrentGhost : listOfGhostLayouts) 
				{
					Ghost currentGhost = (Ghost)layoutForCurrentGhost.getChildAt(0);
					
					if(currentBullet.getxCoordinate() == currentGhost.getxCoordinate() && currentBullet.getyCoordinate() == currentGhost.getyCoordinate() && currentGhost.isVisible && currentBullet.isVisible)
					{
						Log.d("Ghost took damage", "We SHould be killing some ghotsts right now");
						currentGhost.changeHealth(currentBullet.getDamage() * -1);
				...
				if(currentBullet.getxCoordinate() <= 0 || currentBullet.getxCoordinate() >= 9 || currentBullet.getyCoordinate() <= 0 || currentBullet.getyCoordinate() >= 9)
				{
					currentBullet.setVisible(false);
					gl.removeView(layoutForCurrentBullet);
					listOfBulletLayouts.remove(currentBullet);
					continue;
